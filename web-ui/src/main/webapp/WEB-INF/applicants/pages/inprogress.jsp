<!doctype html>
<html lang="us">
<head>
    <meta charset="utf-8">
    <title>Group status: in progress/offering</title>
    <link href="http://code.jquery.com/ui/1.11.0/themes/ui-lightness/jquery-ui.css" rel="stylesheet">
    <link href="applicants/css/applicants.css" rel="stylesheet">
    <script type="text/javascript" src="http://code.jquery.com/jquery-1.10.2.js"></script>
    <script type="text/javascript" src="http://cdnjs.cloudflare.com/ajax/libs/mustache.js/0.8.1/mustache.js"></script>
    <script src="applicants/js/handler.js"></script>
    <script src="applicants/js/inprogress-handler.js"></script>
    <script type="text/javascript" src="http://code.jquery.com/ui/1.11.0/jquery-ui.js"></script>
    <script src="applicants/js/jquery.nicefileinput.min.js"></script>
</head>
<body>
<button id="submitList" class="scheduleView">Submit</button>
<br>

<div class="accordion interviewed"></div>
<div id="dialog" title="Information">
    <p></p>
</div>
</body>
</html>
